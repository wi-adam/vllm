...
def test_reasoning_swallows_post_think_content(qwen3_tokenizer):
    """
    Regression test: If </think> was ALREADY emitted, and a later delta
    contains <tool_call>, the parser incorrectly sends the text BEFORE
    <tool_call> as reasoning instead of content.
    """
    ...
    # Delta 2: content + tool call in same delta (e.g. speculative decoding
    # or fast generation). The text is clearly AFTER </think>, so it must be
    # content.
    vocab = qwen3_tokenizer.get_vocab()
    tool_call_id = vocab["<tool_call>"]
    ...
    assert msg2.reasoning is None, (
        f"Parser incorrectly swallowed post-think text into reasoning: {msg2.reasoning}"
    )
...
def test_extract_reasoning_streaming_fragmented_end_and_tool_call(qwen3_tokenizer):
    ...
    if msg2 and msg2.content:
        assert msg2.content != "ol_call>\n<function=", (
            "Parser corrupted the tool call tag by splitting it across "
...
def test_parse_delta_reasoning_not_bypassed_when_prompt_has_tool_examples(
    ...
    reasoning_token = qwen3_tokenizer.encode(reasoning_text, add_special_tokens=False)[
        0
    ]
